...
export function DataTable({ data, types, smartTypes = {} }) {
    ...
    const filteredColumns = useMemo(() => {
        ...
        return allColumns.filter(col => col.toLowerCase().includes(term));
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-4">
                ...
                <div className="flex items-center gap-3 relative">
                    ...
                    {isColumnDropdownOpen && (
                        <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-30 p-2 max-h-60 overflow-y-auto ring-1 ring-black/5">
                            ...
                            {filteredColumns.map(col => (
                                <label key={col} className="flex items-start gap-2 px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded cursor-pointer group">
                                    <input
                                        type="checkbox"
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <span className="text-sm text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white" title={col}>
                                                <span className="block font-medium break-words">{column}</span>
                                                {table && (
                                                    <span className="block text-[10px] text-gray-400 dark:text-gray-500 break-words">{table}</span>
                                                )}
                                            </span>
                                        );
                                    })()}
